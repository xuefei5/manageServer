...
import com.simplemall.account.dal.SysStaticDataMapper;
...
public class DuplicateCheckController {
	...
	public Result<Object> doDuplicateCheck(DuplicateCheckVo duplicateCheckVo, HttpServletRequest request) {
		...
		if (StringUtils.isNotBlank(duplicateCheckVo.getDataId())) {
			// [2].编辑页面校验
			num = sysStaticDataMapper.duplicateCheckCountSql(duplicateCheckVo);
		} else {
			// [1].添加页面校验
			num = sysStaticDataMapper.duplicateCheckCountSqlNoDataId(duplicateCheckVo);
		}
